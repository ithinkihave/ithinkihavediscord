...
async function replyToReferencedMessage(
	...
	try {
		...
		console.error(
			"[bot] error replying to referenced message for truth question",
			error,
		);
		return false;
	}
...
function isOutsideTruthCheckChannel(message: DiscordMessage): boolean {
	if (
		!message.guild ||
		!TRUTH_CHECK_RESTRICTED_GUILD_IDS.has(message.guild.id)
	) {
		return false;
	}
	...
	if (
		...
		message.channel.name === TRUTH_CHECK_CHANNEL_NAME
	) {
		return false;
	}
	...
	if (
		...
		"name" in message.channel.parent
	) {
...
export async function handleTruthQuestion(
	message: DiscordMessage,
): Promise<void> {
	...
	const repliedToReference = await replyToReferencedMessage(
		message,
		randomResponse,
	);
