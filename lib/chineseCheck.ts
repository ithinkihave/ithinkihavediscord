...
function shouldDeleteDisallowedChineseChannelMessage(
...
export function shouldDeleteChineseChannelContent(text: string): boolean {
	const textWithoutMentions = text.replace(
		ALLOWED_CHINESE_CHANNEL_MENTION_REGEX,
		"",
	);
	...
	if (
		chineseCharacterCount / countableCharacterCount <
		CHINESE_CHARACTER_RATIO_THRESHOLD
	) {
		return true;
	}
	...
	for (const symbol of text) {
		...
		if (
			nonWhitespaceCharacterCount >=
				ASCII_ART_MIN_NON_WHITESPACE_LENGTH &&
...
function isSuspiciousRepeatedCharacterArt(text: string): boolean {
	...
	return (
		maxRepeatedCharacterCount >= ASCII_ART_REPEATED_CHARACTER_MIN_COUNT &&
		maxRepeatedCharacterCount / nonWhitespaceCharacterCount >=
			ASCII_ART_REPEATED_CHARACTER_RATIO_THRESHOLD
	);
}

export async function handleChineseChannelEnglishCheck(
	message: DiscordMessage,
): Promise<boolean> {
