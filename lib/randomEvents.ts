...
export async function handleRandomEvents(message: DiscordMessage): Promise<void> {
  if (shouldReact()) {
  ...
  if (Math.random() <= REACT_PROBABILITY) {
    const reply = MARKETPLACE_REPLIES[Math.floor(Math.random() * MARKETPLACE_REPLIES.length)];
    if (!reply) return;
